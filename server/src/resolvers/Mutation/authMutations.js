// import { Context } from '../../../node_modules/graphql-authentication/dist/utils';
...
const utils_1 = require('../../../node_modules/graphql-authentication/dist/utils')
...
exports.authMutations = {
  async signupByInvite(parent, { data }, ctx) {
    ...
    return {
      token: generateToken(updatedUser, ctx),
      user: updatedUser
    }
  },
  ...
  async login(parent, { email, password }, ctx) {
    ...
    // Add Session to user
    if (ctx.db && user) {
      const userSession = await ctx.db.query.user({ where: { id: user.id } })
      if (!userSession.sessionId)
        user = await ctx.graphqlAuthentication.adapter.updateUserInfo(ctx, user.id, { sessionId: uuid() })
      else
        user = userSession

      let token = generateToken(user, ctx)
      return {
        token: token,
        user
      }
  ...
  async inviteUser(parent, { data }, ctx) {
    ...
    // check if it's a re-invite or user still needs to setup account
    if (user.inviteAccepted === false) {
      // Send email
      if (ctx.graphqlAuthentication.mailer) {
        ctx.graphqlAuthentication.mailer.send({
          template: 'inviteUser',
          ...
          locals: {
            ...
            email: user.email,
            senderEmail: loggedinUser.isSuper ? '' : 'by ' + loggedinUser.email
          }
        })
      }

      // Send Slack notification when configured.
      const slackInviteWebhookUrl = process.env.SLACK_INVITE_WEBHOOK_URL

      if (process.env.NODE_ENV === 'production' && slackInviteWebhookUrl) {

        let payload = {
          text:
            data.email.toLowerCase() +
            ' has been invited to register an account on ' +
            process.env.APP_DOMAIN + ' by ' + loggedinUser.email
        }

        axios.post(slackInviteWebhookUrl, payload, {
          headers: {
